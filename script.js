...
function initTabs() {
  ...
  /* Activate tab from URL hash, or first tab by default */
  var hash = window.location.hash;
  var hashTarget = hash ? hash.replace('#', '') : '';
  var activated = false;

  if (hashTarget) {
    for (var m = 0; m < tabBtns.length; m++) {
      if (tabBtns[m].getAttribute('data-tab') === hashTarget) {
        tabBtns[m].onclick.call(tabBtns[m]);
        activated = true;
        break;
      }
    }
